...
namespace Client.Services
{
    public class DeleteDataServices
    {
        ...
        // Xoá TicketTag
        public async Task DeleteTicketTagAsync(TicketTag ticketTag)
        {
            using (var _context = new AppDbContext())
            {
                // Kiểm tra xem TicketTag có tồn tại trong DB không
                var item = _context.TicketTags.FirstOrDefault(x => x.Id == ticketTag.Id);
                if (item != null)
                {
                    _context.TicketTags.Attach(item);
                    _context.TicketTags.Remove(item);
        ...
        // Xóa SpareParts
        public async Task DeleteSparePartsAsync(SpareParts spareParts)
        {
            using (var _context = new AppDbContext())
            {
                // Kiểm tra xem SpareParts có tồn tại trong DB không
                var item = _context.SpareParts.FirstOrDefault(x => x.Id == spareParts.Id);
                if (item != null)
                {
                    _context.SpareParts.Attach(item);
                    _context.SpareParts.Remove(item);
                    await _context.SaveChangesAsync();
                }
                else
                {
        ...
        // Xóa TicketSparePart
        public async Task DeleteTicketSparePartAsync(TicketSpareParts ticketSpareParts)
        {
            using (var _context = new AppDbContext())
            {
                // Kiểm tra xem TicketSpareParts có tồn tại trong DB không
                var item = _context.TicketSpareParts.FirstOrDefault(x => x.Id == ticketSpareParts.Id);
                if (item != null)
                {
                    _context.TicketSpareParts.Attach(item);
                    _context.TicketSpareParts.Remove(item);
                    await _context.SaveChangesAsync();
                }
                else
                {
        ...
        //Xóa Dealer
        public async Task DeleteDealerAsync(Dealer dealer)
        {
            using (var _context = new AppDbContext())
            {
                // Kiểm tra xem dealer có tồn tại trong DB không
                var item = _context.Dealers.FirstOrDefault(x => x.Id == dealer.Id);
                if (item != null)
                {
                    _context.Dealers.Attach(item);
        ...
        public async Task DeleteExpenseAsync(Expense expense)
        {
            using (var _context = new AppDbContext())
            {
                // Kiểm tra xem Expense có tồn tại trong DB không
                var item = _context.Expenses.FirstOrDefault(x => x.Id == expense.Id);
                if (item != null)
                {
                    _context.Expenses.Attach(item);
