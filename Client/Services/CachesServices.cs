...
namespace Client.Services
{
    ...
    public class CachesServices
    {
        ...
        private List<SpareParts> _spareParts;
        public List<SpareParts> SpareParts
        {
            get => _spareParts ?? (_spareParts = new List<SpareParts>());
        ...
        {
            get => _dealers ?? (_dealers = new List<Dealer>());
            set => _dealers = value;
        }
        #endregion
        //-------------------------------------------------------------------------------------
        ...
        private List<TicketTag> _ticketTags;
        public List<TicketTag> TicketTags
        {
            get => _ticketTags ?? (_ticketTags = new List<TicketTag>());
        ...
        {
            get => _currentTicketNumber;
            set => _currentTicketNumber = value;
        }
        ...
        public int NextTicketNumber
        {
            ...
            set => _nextTicketNumber = value;
        }
        private List<TicketSpareParts> _ticketSpareParts;
        public List<TicketSpareParts> TicketSpareParts
        {
            get => _ticketSpareParts ?? (_ticketSpareParts = new List<TicketSpareParts>());
            set => _ticketSpareParts = value;
        }
        private List<TicketServiceType> _ticketServiceTypes;
        public List<TicketServiceType> TicketServiceTypes
        {
            get => _ticketServiceTypes ?? (_ticketServiceTypes = new List<TicketServiceType>());
            set => _ticketServiceTypes = value;
        }
        #endregion
        //-------------------------------------------------------------------------------------
        ...
        private static SystemRole _systemRole;
        ...
        {
            get => _systemRoles ?? (_systemRoles = new List<SystemRole>());
        ...
        public async Task GetAllData()
        {
            ...
            _ticketSpareParts = await _context.TicketSpareParts.ToListAsync();
            _ticketTags = await _context.TicketTags.ToListAsync();
            _ticketServiceTypes = await _context.TicketServiceTypes.ToListAsync();
            int ticketCount = await _context.Tickets.CountAsync();
            if (ticketCount > 0)
            {
                _currentTicketNumber = await _context.Tickets.MaxAsync(t => t.TicketNumber);
                _nextTicketNumber = _currentTicketNumber + 1;
            }
            else
            {
                _currentTicketNumber = 0;
                _nextTicketNumber = 1;
            }
            // Model Entities
            ...
            _spareParts = await _context.SpareParts.ToListAsync();
            // Customer Entities
            _contacts = await _context.Contacts.ToListAsync();
            _customers = await _context.Customers.ToListAsync();
            _dealers = await _context.Dealers.ToListAsync();
            // Task Entities
            ...
            _userRoles = await _context.UserRoles.ToListAsync();
            _systemRoles = await _context.SystemRoles.ToListAsync();
            // Expense Entities
            _expenses = await _context.Expenses.ToListAsync();
        ...
        public async Task GetCurrentUserInfo(int userId)
        {
            _userId = userId;

            // Combine both queries into a single join to reduce round-trips
            var result = await (from ur in _context.UserRoles
                                join sr in _context.SystemRoles on ur.RoleId equals sr.Id
                                where ur.UserId == userId
                                select sr).FirstOrDefaultAsync();

            _systemRole = result ?? new SystemRole { Id = 0, Name = "Guest" };
        }
